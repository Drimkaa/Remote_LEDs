...
class FireAnimation extends StatefulWidget {
  const FireAnimation({super.key, required this.colors, this.duration = const Duration(seconds: 0, milliseconds: 200)});
...
class _FireAnimationState extends State<FireAnimation> {
  ...
  void initState() {
    ...
    _colors = [getColor(), getColor(), getColor(), getColor()];
    _timer = Timer.periodic(widget.duration ~/ 10, _updateTimer);
  }

  void updateColors() {
    baseHue = HSLColor.fromColor(widget.colors[0]).hue;
    setState(() {
      _colors = [getColor(), getColor(), getColor(), getColor()];
    });
  }
